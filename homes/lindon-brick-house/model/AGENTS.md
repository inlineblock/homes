...
- Keep `facade_finish()` as the common finish map for walls and exposed floor/ceiling edges. The entire front stacked-window bay and gable are charcoal; the lower surrounding level is brick and the upper volumes use charcoal or `mushroom-mineral-plaster/v001`. Inspect both arrival cameras so a brick seam or pale corner does not split the dark bay again.
- The study roof is unoccupied. Preserve its positive fall and slim flush upstand; do not treat the coping as a guard or invent a terrace door without a coordinated program/plan change.
