# Concealed cream inset dishwasher, 24 inch
...
Link the closed collection `cream-inset-dishwasher-24in | v001` from `cream-inset-dishwasher-24in.blend`. The explicit open/service collection is `cream-inset-dishwasher-24in | open v001`; link only one operating state at a time.
...
Source: `tools/library/build_concealed_kitchen_appliances.py`. Derivation: `appliances/dishwasher-24in` / `v001`. CC BY 4.0, attribution Homes project contributors; source code is MIT. Preserve immutable published versions.
