# Cream inset refrigerator / freezer, 48 inch
...
Link the closed collection `cream-inset-fridge-freezer-48in | v001` from `cream-inset-fridge-freezer-48in.blend`. The explicit open/service collection is `cream-inset-fridge-freezer-48in | open v001`; link only one operating state at a time.
...
Source: `tools/library/build_concealed_kitchen_appliances.py`. Derivation: `appliances/panel-ready-fridge-48in` / `v001`. CC BY 4.0, attribution Homes project contributors; source code is MIT. Preserve immutable published versions.
