...
def publish(root,slug,m,description):
    ...
    if not path.exists():
        ...
        meta={'schema_version':1,'id':'materials/'+slug,'version':'v001','name':m.name,'units':'meters','license':'CC-BY-4.0','rights':'Original; attribution Homes project contributors','source':{'kind':'original','generator':'tools/common/finish_palette.py'},'files':{'blender':path.name},'dependencies':[],'description':description,'texture_scale':'Object coordinates in meters; no external images.'}
