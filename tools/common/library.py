...
def publish_material(root,slug,material,metadata):
    ...
    if not path.exists():
        ...
        metadata.update({'schema_version':1,'id':f'materials/{slug}','version':'v001','units':'meters','source':{'kind':'original','generator':'tools/timber02/build.py'},'rights':'Original project asset; no redistribution license assigned','files':{'blender':path.name}})
