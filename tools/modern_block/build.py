...
from utils import *
import design as d
from envelope import build_envelope,wall,cut,roof,frame
...
points=[(x,y,GROUND_CEILING) for x in [12.0,16,20] for y in [2.5,6.0,9.5] if not (d.STAIR_VOID[0]<x<d.STAIR_VOID[2] and d.STAIR_VOID[1]<y<d.STAIR_VOID[3])]
points += [(x,y,UPPER_CEILING) for x,y in [(11.5,2),(15.3,2),(11.5,5.5),(15.3,5.5),(18.5,2),(20.5,2),(18.5,4.3),(12,9),(12,13),(12,16),(19,14),(20.7,16)]]
points += [(-5,y,GROUND_CEILING) for y in [2,7,11]]
for x,y,z in points:
...
lanai_posts=[.1,2,4,6,8,9.9]
for x in lanai_posts:tag(box('Lanai slender steel post',(x,0,(d.GROUND+d.GROUND_ROOF_Z)/2),(.09,.09,d.GROUND_ROOF_Z-d.GROUND),M['dark'],.01),'IfcColumn')
# The rigid three-meter pivot leaves remain unchanged. A fixed clerestory
# fills the higher roofline rather than stretching the moving assemblies.
tag(box('Lanai pivot header',(5,0,d.GROUND+3.17),(9.89,.10,.10),M['dark'],.006),'IfcBeam')
transom_bottom=d.GROUND+3.22
transom_window=linked_collection(ROOT,'openings','slim-dark-window','v002')
for index,(x0,x1) in enumerate(zip(lanai_posts,lanai_posts[1:]),1):
 frame('Lanai fixed clerestory '+str(index),(x0+.045,0),(x1-.045,0),transom_bottom,GROUND_CEILING-transom_bottom,M,transom_window,'Ground floor')
 OPENINGS.append({'name':'Lanai fixed clerestory '+str(index),'a':[x0+.045,0],'b':[x1-.045,0],'z':transom_bottom,'height':GROUND_CEILING-transom_bottom,'kind':'window','level':'Ground floor'})
for i in range(5):
 obj=instance('Pivot screen '+str(i+1),assets['louver'],(1+i*2,0,d.GROUND+.12),math.radians(75));obj['operating_state']='Open 75 degrees';obj['closed_z_rotation']=0.0;obj['open_z_rotation']=math.radians(75)
# Lanai furniture leaves 1.05m unobstructed behind leaf sweep.
instance('Lanai sofa',assets['outdoor_sofa'],(4.9,3.9,d.GROUND),math.pi)
instance('Lanai lounge chair',assets['outdoor_chair'],(1.8,3.6,d.GROUND),-.5)
for y in [10.3,14.2]:instance('Pool chaise',assets['chaise'],(1.35,y,d.GROUND+.01),0)
...
box('Balcony floor',(8.5,13,d.UPPER-.12),(3,8,.24),M['stone'],.012)
for x,y in [(7,9),(7,17)]:tag(box('Balcony slender support',(x,y,(d.GROUND+d.UPPER_ROOF_Z)/2),(.12,.12,d.UPPER_ROOF_Z-d.GROUND),M['dark'],.01),'IfcColumn')
roof('Covered balcony',(6.9,9,10,17.1),d.UPPER_ROOF_Z,M,'Upper floor')
for a,b in [((7,9),(7,17)),((7,9),(10,9)),((7,17),(10,17))]:
 wall('Balcony glass guard',a,b,d.UPPER,1.06,M['glass'],.016,'Upper floor');rod('Balcony guard cap',(a[0],a[1],d.UPPER+1.07),(b[0],b[1],d.UPPER+1.07),.016,M['bronze'])
for y in [10.6,15]:instance('Balcony lounge chair',assets['outdoor_chair'],(8.4,y,d.UPPER),math.pi/2)
...
CAMS={
 '13-east-arrival':((38,-18,2.8),(19,6,3.5),35),
 '01-front-arrival':((28,-34,2.3),(8,3,3.1),43),
 '02-courtyard-pool':((2.1,24,3.1),(11,6,3.2),26),
 '03-roof-and-site':((-30,34,28),(6,9,2.0),42),
 '04-lanai-open':((9,-7,1.8),(4.6,4,1.95),26),
 '05-kitchen':((14.3,12.5,1.62),(12.8,17.2,1.35),21),
 '14-kitchen-wall':((12.85,15.70,1.72),(12.95,19.5,1.38),18),
 '15-kitchen-open':((12.85,15.70,1.72),(12.95,19.5,1.38),18),
 '16-kitchen-island':((15.6,18.4,1.68),(12.9,15.5,1.10),21),
 '17-pantry-open':((16.45,13.15,1.65),(17.4,15.5,1.35),18),
 '06-living':((17.8,6.6,1.62),(18.7,3.3,1.25),20),
 '07-primary-bedroom':((16.1,6,d.UPPER+1.60),(13.2,2.7+d.UPPER_FRONT/2,d.UPPER+1.05),24),
 '08-primary-bath':((17.4,1.95,d.UPPER+1.65),(20,3.8,d.UPPER+1.05),20),
 '09-lanai-closed':((9,-7,1.8),(4.6,4,1.95),26),
 '10-pool-and-balcony':((1,19,2.7),(9.8,11.5,3.6),25),
 '12-bath-tub-shower':((17.55,4.4,d.UPPER+1.55),(20.2,1.3,d.UPPER+.90),22),
 '11-stair-gallery':((16.1,10,d.UPPER+1.60),(20.5,10.7,d.UPPER+.80),22),
}
...
area('Broad front fill',(9,-8,14),(8,5,3.4),1800,15,(1,.97,.92))
for x,y in [(5,3),(12.5,15),(16,4),(20,2)]:area('Soft interior bounce',(x,y,GROUND_CEILING-.35),(x,y,d.GROUND),85,2.5,(1,.87,.70))
area('Primary bedroom soft bounce',(13.2,3+d.UPPER_FRONT/2,UPPER_CEILING-.30),(13.2,3+d.UPPER_FRONT/2,d.UPPER),85,2.5,(1,.87,.70))
area('Primary bath soft daylight',(20,2,UPPER_CEILING-.40),(19.6,3,d.UPPER+.65),320,3.0,(1,.96,.91))
area('Primary bath vanity light',(18.7,3.9,UPPER_CEILING-.40),(18.7,4.5,d.UPPER+.90),110,1.8,(1,.89,.76))
...
meta={'schema_version':1,'id':d.SLUG,'name':'Modern Block','status':'architectural concept; dimensions and unseen layout interpreted','units':'meters','display_units':'feet-inches','bedrooms':4,'bathrooms':3.5,'gross_enclosed_area_sqft':round((d.CONDITIONED_M2+d.GARAGE_M2)/.3048**2,4),'conditioned_gross_area_sqft':round(d.CONDITIONED_M2/.3048**2,1),'garage_area_sqft':round(d.GARAGE_M2/.3048**2,1),'area_basis':'Gross occupied floor plates including wall footprints, minus upper stair void; gross enclosed adds garage. Excludes courtyard, pool, lanai, balcony, roof and exterior terraces. Concept dimensions.','storeys':[{'name':'Ground floor','elevation_m':d.GROUND,'clear_ceiling_height_m':d.GROUND_CLEAR},{'name':'Upper floor','elevation_m':d.UPPER,'clear_ceiling_height_m':d.UPPER_CLEAR}],'upper_front_projection_m':-d.UPPER_FRONT,'ceiling_exceptions':[{'area':'Rear service rooms beyond the kitchen slats','clear_ceiling_height_m':d.GROUND_CLEAR+.12,'reason':'Recessed rear roof backing; kitchen slat undersides remain at the ground clear-height datum.'}],'software':{'blender':bpy.app.version_string,'bonsai':'0.8.5','ifcopenshell':'0.8.5'},'asset_dependencies':deps,'deliverables':{'presentation_model':'model/modern-block.blend','building_model':'model/modern-block.ifc','gallery':'outputs/README.md','ground_plan':'outputs/plans/ground-floor.png','upper_plan':'outputs/plans/upper-floor.png'},'authoring':{'build':'Blender --background --python tools/modern_block/build.py','render':'Blender --background homes/modern-block/model/modern-block.blend --python tools/modern_block/render.py','plans':'python3 tools/modern_block/draw_plans.py'}}
