...
for a,b in [(0,3),(15,18),(22,24),(36,42),(54,68)]:wall('Front mineral render',((a+b)/2,.20,5.25),(b-a,.40,10.5))
for a,b in [(3,15),(24,36),(42,54)]:
...
wall('Front entrance head',(20,.20,9.5),(4,.40,2))
...
for x,label in [(0.20,'West'),(67.80,'East')]:
    ...
    else:glass_wall(label+' great room glazing',(x,23),(x,38.2),9.5,M['glass'],M['bronze'],.10,4)
...
for name,a,b,opens in WALLS:
    ...
    for off,w in opens:
        if ux:loc=(a[0]+off+.09,a[1]+(w/2 if name=='Primary bath front' else -w/2),3.75);sz=(.13,w-.16,7.5)
        else:loc=(a[0]+(-w/2 if name=='Primary east' else w/2),a[1]+off+.09,3.75);sz=(w-.16,.13,7.5)
...
collection('09 Roof | finished ceiling and deep eaves')
box('Thin insulated roof concept',(34,20,10.88),(72,47.5,.70),M['plaster'],.025)
box('Continuous warm white ceiling',(34,20,10.48),(71.5,47.2,.06),M['ceiling'],.012)
box('Roof shadow reveal',(34,20,10.60),(72.08,47.58,.10),M['bronze'],.008)
# A taller, quiet entrance volume breaks up the long low silhouette.
box('Entry raised canopy',(20,-1.5,11.15),(7,9,.32),M['stone'],.03)
# Oak soffit slats run across the rear canopy, not through the living-room ceiling.
for i in range(170):box('Rear oak soffit slat',(.1+i*.4,41.90,10.38),(.36,3.65,.10),M['oak'],.007)
for x in [44,65]:
    box('Linear soffit LED',(x,41.9,10.29),(5.5,.04,.03),M['glow'],.004)
...
for ix in range(19):
    for iy in range(5):box('Terrace limestone paver',(-2+(ix+.5)*3.9,40+(iy+.5)*3.6,-.03),(3.88,3.58,.08),M['stone'],.009)
...
box('Illustrative dune ground',(34,-32.5,-1.25),(400,225,1.3),M['sand'])
...
for i in range(110):
    ...
    grasses('Dune grass drift',x,y,rng.uniform(.55,1.5),M['grass'],i+400)
    if i%4==0:shrub('Low coastal planting',x,y,rng.uniform(.8,1.8),M['leaf'],i+800)
for x,y,h in [(-8,35,14),(77,29,13),(-8,8,17),(77,-1,15)]:tree('Wind-shaped olive',x,y,h,M['bark'],M['leaf'],int(x+y+200))
for i in range(9):
    tree('Coastal background canopy',-12+i*11,-8-rng.uniform(0,10),rng.uniform(16,24),M['bark'],M['leaf'],1400+i)
for i in range(35):
    x=rng.choice([rng.uniform(-8,-2),rng.uniform(71,80)]);y=rng.uniform(-4,56)
    grasses('Border grasses',x,y,rng.uniform(.8,1.7),M['grass'],i+1000)
    if i%3==0:rock('Weathered pale stone',x,y,1.4,M['stone'],i+12)
# Front entry path.
for y in [-4,-9,-14,-19]:box('Entry stepping slab',(20,y,-.03),(6,4.5,.12),M['stone'],.018)
kitchen(M);rooms(M)
