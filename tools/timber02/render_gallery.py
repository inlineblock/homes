...
Omit --draft for 2400 x 1600 / 256-sample candidates; omit names for all ten views.
Promote after QA.
"""
...
draft='--draft' in args;names=[x for x in args if not x.startswith('--')] or list(RENDERS)
...
for slug in names:
    ...
    s.view_settings.exposure=.3 if slug in ['05-living-dining','06-kitchen','08-cooking-wall','09-pantry','10-living-room'] else (.5 if slug=='07-rear-garden' else base_exposure)
    ...
    for c in bpy.data.collections:
        if c.name.startswith('03 Ceiling |'): c.hide_render=cut
    if cut:
