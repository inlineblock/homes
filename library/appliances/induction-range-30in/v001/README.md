# Original 30-inch induction range with single oven

![Closed native preview](preview.png)

![Door open native preview](preview-open.png)

Link only `ASSET_induction-range-30in_v001` from `induction-range-30in.blend`. Origin is the body footprint center on the floor; front is -Y. Exact measured bounds and the opening envelope are in [asset.json](asset.json). Rotate the named oven-door hinge +90 degrees about local X to inspect opening. Never scale an appliance to fit.
