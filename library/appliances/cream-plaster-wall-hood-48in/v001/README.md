# Cream plaster capture hood, 48 inch
...
Link the closed collection `cream-plaster-wall-hood-48in | v001` from `cream-plaster-wall-hood-48in.blend`. The explicit open/service collection is `cream-plaster-wall-hood-48in | open v001`; link only one operating state at a time.
...
Source: `tools/library/build_concealed_kitchen_appliances.py`. Derivation: `appliances/wall-hood-36in` / `v001`. CC BY 4.0, attribution Homes project contributors; source code is MIT. Preserve immutable published versions.
