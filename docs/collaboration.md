...
5. Review results in ignored `outputs/work/`, then replace the stable files in `outputs/images/` or `outputs/plans/`. Update the outputs gallery and project manifest, and commit source plus current reviewed deliverables. Git holds previous revisions; do not keep output history folders. Temporary renders, caches, and automatic Blender backups are ignored.
