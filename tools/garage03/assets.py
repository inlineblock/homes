...
from design import PLATFORM_WIDTH,PLATFORM_LENGTH,PLATFORM_SPACING,POOL_PLAY,CAR_CENTERS,LIFT_CENTER,LIFT_ASSET_VERSION

def publish(root,category,slug,make,metadata,version="v001"):
    folder=root/'library'/category/slug/version;path=folder/f'{slug}.blend'
    if not path.exists():
        folder.mkdir(parents=True,exist_ok=True);c=collection(slug+' | original '+version);make()
        bpy.data.libraries.write(str(path),{c},fake_user=True)
        metadata.update({'schema_version':1,'id':category+'/'+slug,'version':version,'units':'meters','origin':'Floor/driveway level at plan center','license':'CC-BY-4.0','rights':'Original project asset; CC BY 4.0; attribution: Homes project contributors','source':{'kind':'original','generator':'tools/garage03/assets.py'},'files':{'blender':path.name},'dependencies':[]})
...
def lift_asset(root,M):
    def make():
        ...
        for level,z in [('upper',0),('lower',-PLATFORM_SPACING)]:
            ...
            for x in [v-LIFT_CENTER[0] for v in CAR_CENTERS]:box(level+' wheel stop',(x,PLATFORM_LENGTH/2-.7,z+.11),(4.5,.16,.22),M['black'],.018)
    ...
    return publish(root,'fixtures','double-pit-parking-carriage',make,{'change_from_v001':'Adjustable wheel-stop centers shift toward the passenger side for driver-side walk-off; v001 remains immutable.','name':'Double pit parking carriage | schematic','dimensions_m':[PLATFORM_WIDTH*F,PLATFORM_LENGTH*F,(PLATFORM_SPACING+.5)*F],'planning_reference':'KLAUS MultiBase 2072i DP, original simplified proxy; not manufacturer geometry','clear_platform_width_m':5.4,'platform_spacing_m':PLATFORM_SPACING*F,'limitations':'No certified mechanism, anchors, rated loads, or fabricated details. Fixed guide columns are separate building placeholders.'},version=LIFT_ASSET_VERSION)

def pool_asset(root,M):
    def make():
        felt=material('Deep petrol billiard cloth',(.012,.07,.055),.92)
        for x in [-1.65,1.65]:
        ...
        for x in [-1.81,1.81]:
            for y in [-3.62,0,3.62]:cyl('Billiard pocket',(x,y,2.766),.24,.04,M['black'])
        for x in [-2.1,2.1]:
    ...
    return publish(root,'furniture','walnut-eight-foot-pool-table',make,{'name':'Walnut eight-foot pool table','dimensions_m':[4.7*F,8.35*F,2.94*F],'playing_surface_m':[POOL_PLAY[0]*F,POOL_PLAY[1]*F],'cue_length_m':58*.0254,'dimensions_basis':'Original visualization design; 44 x 88 inch playing surface; not a purchasable product'},version='v002')
