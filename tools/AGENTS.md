...
- `common/write_catalog.py` owns the compact root README: one photographic hero and one main/ground plan per home, side by side, linked to its landing page. Per-home gallery writers must call this shared writer instead of inserting full galleries into the root. The gallery verifier keeps full coverage checks on home pages and checks only the compact card contract at the root.
