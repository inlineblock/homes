...
The refreshed [IFC receipt](model/ifc-validation.json) reports IFC4 with zero schema errors; [Bonsai receipt](model/bonsai-validation.json) reports a successful fresh import with 1,091 mesh objects. These validate export/import, not construction or room function. Earlier [model](model/model-validation.json), [gallery-camera](model/gallery-camera-validation.json) and [gallery-output](model/gallery-output-validation.json) receipts retain their own scope; old library/camera counts do not supersede current kitchen validation.
...
The revised [native walkthrough](outputs/videos/timber-courtyard-native.mp4) is promoted with fifteen current paired photographic checkpoints. Its 721 frames fully decode at 1280 × 720, 24 fps and 30.041667 seconds. Sixty-one half-second samples and eight high-sample keyframes were visually reviewed; no visible architecture or furniture crossing was found. A 700 mm body sweep across every camera pose and connecting segment clears 2,063 evaluated fixed meshes, with about 142 mm minimum additional bounds clearance. See the [video receipt](outputs/videos/video-verification.json) for exclusions and exact hashes. GUI playback of this revision was unavailable while the Mac was locked. **The refreshed photographic AI film remains pending generation and review.** The independent measured kitchen review, completed image/plan review and video review are separate evidence.
