"""Dimensioned concept plan and seven-page visual booklet from current model renders."""
import sys,math
...
rect(3,26.6,20,12,'#f0eee7');rect(7.3,26.95,11.4,3.5,SAND,OAK,1);rect(5.95,29.95,3.5,5.3,SAND,OAK,1);rect(11.1,32,5.8,3.4,OAK)
...
d.text(180,973,'Primary suite: 18 x 12 ft bedroom / 6 x 6 ft dressing / L-shaped bath with separate tub, shower, WC and dual vanity',14,MUTED)
...
header(d,5,'A complete arrival','Two covered parking spaces, separate entry walk and a low pitched metal roof.')
...
d.text(550,848,'Parking, road, planting and shore are illustrative.',18,MUTED)
d.text(550,886,'Vehicle envelope and walking access checked in model.',18,MUTED)
...
d.text(55,766,'The entry is twice its original planning width, with a full-height sidelight.',23,INK,bold=True)
d.text(55,810,'The first guest bedroom shifts to a 15 x 12 ft planning zone; its closet runs along the east wall.',18,MUTED)
d.text(55,852,'A short, open arrival joins the hall and dining area. Furniture and wardrobe access are checked.',18,MUTED)
...
writer=PdfWriter()
for file in ['cover-page.pdf','plan-page.pdf','counter-page.pdf','interior-page.pdf','arrival-page.pdf','entry-page.pdf','suite-page.pdf']:writer.append(PdfReader(WORK/file))
...
assert len(PdfReader(FINAL_PDF).pages)==7
...
header(d,8,'Arrival and roof plan','Illustrative road, two-car parking and pedestrian approach / no surveyed parcel')
SX=lambda x:150+(x+10)*6.5;SY=lambda y:200+(40-y)*6.5
for x,y,w,h,c in [(0,0,68,40,SAND),(73,-27,26,24,OAK),(73,-45,26,18,'#ccccbf'),(-10,-67,120,22,'#8f9996'),(17,-45,6,45,WHITE),(17,-4.25,69,4,WHITE)]:d.rect(SX(x),SY(y+h),w*6.5,h*6.5,c,INK,1)
...
for x in [79.5,92.5]:
    d.rect(SX(x-3.15),SY(-6.5),6.3*6.5,16*6.5,'#e8ece9',INK,1)
