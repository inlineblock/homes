...
from assets import lift_asset,pool_asset
...
finishes.prepare(ROOT,M)
for slug,key in [('warm-vertical-cedar','cedar'),('charcoal-standing-seam','roof')]:
    version='v002' if slug=='warm-vertical-cedar' else 'v001'
    path=ROOT/'library/materials'/slug/version/f'{slug}.blend'
...
for name,rect in [('Ground front',(0,0,36,8)),('Ground rear',(0,27,36,34)),('Ground west',(0,8,3,27)),('Ground east',(25,8,36,27))]:floor(name,*rect,0)['ifc_storey']='Ground floor'
x1,y1,x2,y2=PIT
...
wall('Pit west',2.5,8,3,27,-PIT_FRONT_DEPTH,PIT_FRONT_DEPTH)
wall('Pit east',25,8,25.5,27,-PIT_FRONT_DEPTH,PIT_FRONT_DEPTH)
wall('Pit rear',3,27,25,27.5,-PIT_FRONT_DEPTH,PIT_FRONT_DEPTH)
wall('Pit front',3,7.5,25,8,-PIT_FRONT_DEPTH,PIT_FRONT_DEPTH)
# Exact 2 inch fall from rear to front of pit.
pitched_plate('Pit drainage slope',3,25,8,27,-PIT_FRONT_DEPTH,-PIT_FRONT_DEPTH,M['concrete'],.015)
# Roof helper slopes x, so create the drainage plane explicitly along y.
mesh('Pit finished slope',[(3,8,-PIT_FRONT_DEPTH+.012),(25,8,-PIT_FRONT_DEPTH+.012),(25,27,-PIT_REAR_DEPTH),(3,27,-PIT_REAR_DEPTH)],[(0,1,2,3)],M['concrete'])
box('Pit collection channel',(14,8.12,-PIT_FRONT_DEPTH+.025),(21.4,.16,.05),M['black'])
set_level(c,'Parking pit')
c=collection('01 Architecture | lower shell')
wall('West ground wall',0,0,.5,34,0,GARAGE_CLEAR)
wall('Rear ground wall',.5,33.5,36,34,0,GARAGE_CLEAR)
wall('East ground wall',35.5,0,36,33.5,0,GARAGE_CLEAR)
wall('Stair separation front',27.5,.5,28,1,0,GARAGE_CLEAR)
wall('Stair separation rear',27.5,4.5,28,33.5,0,GARAGE_CLEAR)
wall('Internal door head',27.5,1,28,4.5,7.5,GARAGE_CLEAR-7.5)
# Door shown open into lobby; no swing crosses apron or first stair tread.
wall('Internal stair door open',28,1,31.4,1.14,0,7.4,M['cedar'])['ifc_class']='IfcDoor'
# Pedestrian entrance is separate from lift access.
wall('Entry left pier',25,0,32,.5,0,GARAGE_CLEAR)
wall('Entry right pier',35.5,0,36,.5,0,GARAGE_CLEAR)
wall('Entry lintel',32,0,35.5,.5,8,GARAGE_CLEAR-8)
...
wall('Separate stair entry',32.05,.16,35.45,.34,.02,7.95,M['cedar']);box('Entry vertical pull',(35.1,-.02,3.55),(.06,.16,1.65),M['black'],.02)
box('Entry canopy',(32,-1.8,8.8),(8.5,4.3,.3),M['roof'],.025)
set_level(c,'Ground floor')
...
wall('Upper west lower',0,.5,.5,34,UPPER_FLOOR,3)
wall('Upper west head',0,.5,.5,34,UPPER_FLOOR+8.2,.8)
wall('Upper west rear',0,21,.5,34,UPPER_FLOOR+3,5.2)
glass_wall('Game room west glazing',(.18,.5),(.18,21),UPPER_FLOOR+8.2,M['glass'],M['black'],UPPER_FLOOR+3,5)
wall('Upper rear',.5,33.5,36,34,UPPER_FLOOR,9)
wall('Upper east',35.5,0,36,33.5,UPPER_FLOOR,9)
...
box('Low roof',(18,17,22.85),(39,37,.7),M['roof'],.025)
box('Roof edge fascia',(18,-1.48,22.91),(39,.1,.88),M['black'],.01)
for x in range(-1,38):box('Roof standing seam',(x,17,23.225),(.045,37,.075),M['roof'])
set_level(c,'Game room')
...
for a,b in [((31.6,7,3.0),(31.6,18,UPPER_FLOOR/2+3)),((32,18,UPPER_FLOOR/2+3),(32,7,UPPER_FLOOR+3))]:
...
for y in [7+i*.75 for i in range(21)]:rod('Upper stair guard',(28.05,y,UPPER_FLOOR),(28.05,y,UPPER_FLOOR+3.5),.025,M['black'])
rod('Upper stair guard cap',(28.05,7,UPPER_FLOOR+3.5),(28.05,22,UPPER_FLOOR+3.5),.065,M['black'])
for xa,xb,y in [(28,35.5,22),(28,31.65,7)]:
    for i in range(int((xb-xa)/.25)+1):rod('Upper stair perimeter guard',(xa+i*.25,y,UPPER_FLOOR),(xa+i*.25,y,UPPER_FLOOR+3.5),.022,M['black'])
    rod('Upper stair perimeter cap',(xa,y,UPPER_FLOOR+3.5),(xb,y,UPPER_FLOOR+3.5),.055,M['black'])
set_level(c,'Ground floor')
c=collection('11 Parking | fixed guides and plant')
for x in [4.75,23.25]:
    for y in [LIFT_CENTER[1]-1.5,LIFT_CENTER[1]+1.5]:box('Lift fixed guide column',(x,y,2.15),(.4,.5,15.6),M['black'],.035)
box('Lift hydraulic cabinet',(26.25,30.5,1.75),(1.9,2.5,3.5),M['black'],.06)
box('Lift control pedestal',(26.6,7.15,1.8),(.22,.22,3.6),M['black'])
box('Lift control station',(26.6,7.15,3.8),(.75,.3,1),M['steel'],.04)
box('Emergency stop',(26.6,6.97,3.9),(.18,.08,.18),M['red'],.05)
for i in range(36):box('Pit caution stripe',(3.3+i*.61,7.83,.025),(.32,.3,.025),M['black'] if i%2 else M['terracotta'])
# Solid side/rear screens keep adjacent maintenance areas away from pit edges.
# Front access protection is a supplier-designed enclosure reservation, not certified hardware.
for x in [2.65,25.35]:
    for y in [8,12,17,22,27]:rod('Pit perimeter post',(x,y,0),(x,y,6),.055,M['black'])
    for z in [1,3,6]:rod('Pit perimeter rail',(x,8,z),(x,27,z),.04,M['black'])
    for i in range(76):rod('Pit perimeter infill',(x,8+i*.25,0),(x,8+i*.25,6),.014,M['black'])
for i in range(89):rod('Pit rear screen',(3+i*.25,27.25,0),(3+i*.25,27.25,6),.014,M['black'])
# Transparent front enclosure proxy; opening mechanism/interlocks remain supplier design.
for x in [3,25]:rod('Front enclosure post',(x,7.65,0),(x,7.65,6.3),.065,M['black'])
for z in [.25,3,6]:rod('Front access enclosure rail',(3,7.65,z),(25,7.65,z),.025,M['black'])
for i in range(87):rod('Front access enclosure infill',(3.25+i*.25,7.65,.25),(3.25+i*.25,7.65,6),.01,M['black'])
# Stationary shoulders stop short of the moving deck: front entry aligns with pit y=8.
# No side access from carriage across the pit void is implied.
c=collection('17 Circulation | fixed floor and operator')
route_mat=material('Pedestrian floor marking',(.09,.32,.28),.85)
box('Fixed pedestrian cross aisle',(14,2.5,.009),(26,3.5,.018),route_mat)
box('Operator bay marking',(25.5,6,.009),(3,3,.018),route_mat)
# Human scale figure facing the lift; on fixed floor outside car paths.
for x in [25.1,25.7]:rod('Operator leg',(x,5.9,.1),(x,5.9,2.75),.15,M['black'])
box('Operator torso',(25.4,5.9,3.75),(1.35,.65,1.95),M['fabric'],.2)
human=material('Neutral architectural scale figure',(.33,.32,.3),.72)
sphere('Operator head',(25.4,5.9,5.2),(.3,.32,.41),human)
cyl('Operator neck',(25.4,5.9,4.81),.14,.26,human)
for x in [25.1,25.7]:sphere('Operator shoe',(x,6.04,.16),(.19,.4,.16),M['black'])
rod('Operator upper arm',(25.95,5.9,4.3),(26.18,6.04,3.6),.14,M['fabric'])
rod('Operator forearm',(26.18,6.04,3.6),(26.55,6.95,3.75),.105,M['fabric'])
sphere('Operator hand',(26.55,6.95,3.75),(.13,.2,.1),human)
rod('Operator arm',(24.8,5.9,4.3),(24.85,5.9,3),.13,M['fabric'])
set_level(c,'Ground floor')
...
for i,(x,z) in enumerate([(CAR_CENTERS[0],0),(CAR_CENTERS[1],0),(CAR_CENTERS[0],-PLATFORM_SPACING),(CAR_CENTERS[1],-PLATFORM_SPACING)]):
    for o in car('Car '+str(i+1),x,LIFT_CENTER[1],z,paints[i],M):
        # Face forward into the garage; driver exits to the west side.
        o.location.x=2*x*F-o.location.x;o.location.y=2*LIFT_CENTER[1]*F-o.location.y;o.rotation_euler.z+=math.pi;o['lift_moves']=True
...
box('Media screen',(21.8,33.12,UPPER_FLOOR+5.2),(8.4,.15,4.65),M['black'],.1)
box('Media credenza',(21.8,32.4,UPPER_FLOOR+1.25),(9.2,1.75,2.5),M['walnut'],.04)
for x in [4.7,7.2,9.7]:
    box('Refreshment cabinet',(x,32.25,UPPER_FLOOR+1.5),(2.45,2.25,3),M['walnut'],.025)
box('Refreshment counter',(7.2,32.2,UPPER_FLOOR+3.08),(7.6,2.4,.16),M['stone'],.025)
for x in [4.5,7.2,9.9]:instance('Shared walnut counter stool',stool,(x,29.5,UPPER_FLOOR),math.pi)
...
for i in range(4):rod('Wall cue rack',(1.25+i*.27,32.9,UPPER_FLOOR+1.6),(1.25+i*.27,32.9,UPPER_FLOOR+6.43),.027,M['fir'])
# Small writing/gaming desk in rear stair wing, isolated from cue travel.
box('Gaming desk',(32.5,31.8,UPPER_FLOOR+2.45),(4.8,2.5,.18),M['walnut'],.025)
for x in [30.6,34.4]:box('Gaming desk support',(x,31.8,UPPER_FLOOR+1.2),(.15,2,2.4),M['black'])
set_level(c,'Game room')
c=collection('14 Comfort | dedicated heat pump')
box('Game room heat pump head',(18,33.15,UPPER_FLOOR+7.65),(3.8,.7,1.1),M['white'],.15)
box('Heat pump discharge grille',(18,32.76,UPPER_FLOOR+7.42),(3.3,.025,.18),M['black'],.015)
...
for rect in [(-90,-80,90,0),(-90,34,90,90),(-90,0,0,34),(36,0,90,34)]:floor('Illustrative site',*rect,-.17,.25,M['soil'])
floor('Driveway',1,-35,27,-.05,-.02,.18,M['concrete'])
for y in [-3,-6.5,-10,-13.5]:floor('Entry paver',32,y,35.5,y+2.7,.015,.12,M['stone'])
...
n=world.node_tree.nodes;l=world.node_tree.links;sky=n.new('ShaderNodeTexSky');sky.sky_type='NISHITA';sky.sun_elevation=math.radians(14);sky.sun_rotation=math.radians(250);sky.air_density=.8;sky.dust_density=.2;l.new(sky.outputs[0],n.get('Background').inputs['Color']);n.get('Background').inputs['Strength'].default_value=.22
...
s.render.resolution_x=2560;s.render.resolution_y=1850;s.render.resolution_percentage=100;s.render.image_settings.file_format='PNG'
s.view_settings.view_transform='AgX';s.view_settings.look='AgX - Medium High Contrast';s.view_settings.exposure=-.9
s.render.film_transparent=False
s['visual_standard']='Detailed original geometry, restrained PBR finishes, designed forecourt, 192-sample path-traced renders'
...
meta['area_basis']='36 x 34 ft ground footprint including pit projection, plus upper slab minus 7.5 x 15 ft stair opening; excludes exterior paving and overhangs; not living area'
meta['circulation']={'front_apron_clear_depth_ft':7.5,'cross_aisle_width_ft':3.5,'operator_bay_ft':[3,3],'internal_door_rough_width_ft':3.5,'stair_lobby_depth_ft':6.5,'status':'Spatial reservations; access protection, sightlines with actual vehicles and door ratings require supplier/design coordination'}
for dep in meta['asset_dependencies']:
    if dep['id'] in ['materials/warm-vertical-cedar','furniture/walnut-eight-foot-pool-table']:
        dep['version']='v002';dep['path']='../../library/'+dep['id']+'/v002/'
    if dep['id']=='fixtures/double-pit-parking-carriage':dep['version']=LIFT_ASSET_VERSION;dep['path']='../../library/fixtures/double-pit-parking-carriage/'+LIFT_ASSET_VERSION+'/'
